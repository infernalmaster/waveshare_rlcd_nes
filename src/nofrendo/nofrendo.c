...
/* Leave the emulator for good, from inside the emulation loop.
 *
 * WHY THIS IS NOT main_quit(). Both set console.quit, but main_quit() also
 * ejects, and ejecting calls nes_destroy() - which frees the CPU, the APU, the
 * PPU, the mapper and the ROM. nes_setcontext() copies the machine into the
 * static `nes` BY VALUE, so that static keeps a pointer to every one of those
 * after they are freed. Called from osd_getinput(), which runs inside
 * nes_emulate()'s own loop, that is a use-after-free waiting for someone to add
 * one more line between the input poll and the loop condition.
 *
 * So this sets flags and nothing else. nes_poweroff() ends nes_emulate() at the
 * end of the current frame, console.quit stops main_loop() re-inserting the
 * same cartridge, and whoever called nofrendo_main() does the tearing down once
 * there is provably nothing left running against the memory being freed.
 *
 * WITHOUT THE console.quit HALF, powering off only restarts the game - which is
 * exactly what it did: the loop at the bottom of main_loop() re-inserts
 * console.nextfilename, still the file that was already playing, and the player
 * lands back on its title screen. That loop is upstream's cartridge-swap path,
 * driven by main_insert(), and nothing in this build calls it. */
void main_stop(void)
{
   console.quit = true;
   nes_poweroff();
