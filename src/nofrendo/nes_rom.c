...
/* For NES_PPU_DIAGNOSTICS. Defaulted so this file still builds against an older
 * hw_config.h; included before any use, since a flag that silently reads as 0
...
/* FNV-1a over a loaded region. Exists so two runs of the same file can be
 * compared: the card reader and the flash reader hand nes_rom.c the same bytes
 * or they do not, and nothing else in the log would say which. A checksum that
 * changes from boot to boot is a broken read; one that is stable but the game
 * still locks up is a mapper or a CPU problem, not an I/O one.
 *
 * Behind the flag because it walks the whole image byte by byte out of PSRAM -
 * about 13 ms for a 256K cart, once, on a screen the player is already waiting
 * at. Cheap enough to keep available, not free enough to always pay. */
static uint32 rom_checksum(const uint8 *p, int len)
{
...
#if NES_PPU_DIAGNOSTICS
   log_printf("[ROM] PRG %d bytes sum=%08x   CHR %d bytes sum=%08x\n",
              rom_len, rom_checksum(rominfo->rom, rom_len),
              rominfo->vrom_banks * VROM_BANK_LENGTH,
              rominfo->vrom
                 ? rom_checksum(rominfo->vrom,
                                rominfo->vrom_banks * VROM_BANK_LENGTH)
                 : 0);
#endif
...
rominfo_t *rom_load(const char *filename)
{
   ...
   /* Unconditional, and on Serial rather than through gui_sendmsg, which only
    * reaches a log under NOFRENDO_DEBUG. The mapper number is the first thing
    * worth knowing about a ROM that starts and then stops: it is the one
    * property that varies from game to game while the emulator stays the same. */
   log_printf("[ROM] %s: mapper %d, PRG %dK, CHR %dK, %s%s%s%s\n",
              rominfo->filename, rominfo->mapper_number,
              rominfo->rom_banks * 16, rominfo->vrom_banks * 8,
              (rominfo->mirror == MIRROR_VERT) ? "vert" : "horiz",
              (rominfo->flags & ROM_FLAG_BATTERY)    ? " battery" : "",
              (rominfo->flags & ROM_FLAG_TRAINER)    ? " trainer" : "",
              (rominfo->flags & ROM_FLAG_FOURSCREEN) ? " 4screen" : "");
