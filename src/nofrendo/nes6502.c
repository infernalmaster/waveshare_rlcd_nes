...
#else  
/* SAY SO. A jammed 6502 is the quietest failure this emulator has: PC stops
 * advancing, interrupts are masked off, and nothing else changes - so the
 * emulator thread keeps running its 60 frames a second, vid_flush keeps
 * publishing the same picture, the dither task keeps pushing it, and the input
 * chords keep working. From the outside that is indistinguishable from a
 * healthy emulator showing a still screen, which is exactly how it gets blamed
 * on the display, the card or the PSRAM.
 *
 * One line, once per jam: cpu.jammed is already the "have I said this" flag.
 * The opcode and the address are what say whether the game jumped somewhere it
 * should not have, and where from. */
#define JAM()                                                             \
   {                                                                      \
      PC--;                                                               \
      if (false == cpu.jammed)                                            \
         nofrendo_log_printf("[CPU] JAMMED: illegal opcode $%02X at "     \
                             "$%04X - the 6502 stops here\n",             \
                             bank_readbyte(PC), (unsigned) PC);           \
      cpu.jammed = true;                                                  \
      cpu.int_pending = 0;                                                \
      ADD_CYCLES(2);                                                      \
   }
