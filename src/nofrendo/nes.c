...
/* For NES_RENDER_AFTER_CPU, read in nes_renderframe() below. The sketch folder
 * is on the include path for library sources, which is the same mechanism
 * osd.cpp already relies on. Included rather than left to default, because an
 * undefined flag here would silently keep the old order and look exactly like a
 * setting that had no effect. */
#include "hw_config.h"

#ifndef NES_RENDER_AFTER_CPU
...
static void nes_renderframe(bool draw_flag)
{
   ...
   {
      /* See NES_RENDER_AFTER_CPU in hw_config.h. Only the visible lines move;
       * 241 and 261 carry the VBlank flag and must stay ahead of the CPU, or
       * the NMI below is checked before the flag it depends on is set. */
#if defined(NES_RENDER_AFTER_CPU) && NES_RENDER_AFTER_CPU
      if (nes.scanline >= 240)
      ...
      /* Drawn now that this scanline's writes have been made, so a scroll or a
       * nametable switch issued during the line applies to the line itself
       * rather than to the next one. Still before ppu_endscanline(), which is
       * what advances the vertical counter to the following line. */
#if defined(NES_RENDER_AFTER_CPU) && NES_RENDER_AFTER_CPU
      if (nes.scanline < 240)
         ppu_scanline(vid_getbuffer(), nes.scanline, draw_flag);
#endif
