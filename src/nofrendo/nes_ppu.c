...
/* For NES_PPU_DIAGNOSTICS. The sketch folder is on the include path for library
 * sources, the same way osd.cpp gets it. Defaulted rather than required so this
 * file still builds against an older hw_config.h - but included first, because
 * a flag that silently reads as 0 looks exactly like one that does not work. */
#include "hw_config.h"
...
/* Diagnostics, read by the periodic log line in osd.cpp. A game that polls
 * $2002 for a sprite 0 hit which never arrives keeps running - it just never
 * gets past the poll - so the emulator looks entirely healthy while the picture
 * stands still. These two counters are what tell that apart from a game that
 * has simply decided to show a static screen. */
unsigned long nes_spr0_hits     = 0; /* times sprite 0 struck */
unsigned long nes_ppustat_reads = 0; /* reads of $2002, of any kind */
unsigned long nes_spr0_scanned  = 0; /* scanlines sprite 0 was drawn on */
unsigned long nes_spr0_opaque   = 0; /* ...of those, with an opaque pixel */
unsigned long nes_spr0_colmask  = 0; /* columns where sprite 0 is opaque */
unsigned long nes_spr0_bgmask   = 0; /* columns where the BG under it is solid */

/* WHAT THE RIGHT-HAND EDGE OF ONE SCANLINE WAS BUILT FROM.
 *
 * Sampled on scanline 32, which is inside the eight lines sprite 0 occupies at
 * y=28, so it describes the very rows whose background comes back empty.
 *
 * vaddr carries coarse X in bits 0-4, coarse Y in 5-9 and the nametable in
 * 10-11; xofs is the fine horizontal scroll. Between them they say which tile
 * the last visible pixels are fetched from - with a non-zero xofs the right
 * edge comes from the 33rd tile, the one drawn after the nametable wrap, and
 * that fetch is the only part of this loop a one-screen-mirrored game exercises
 * differently from a vertically-mirrored one.
 *
 * tail packs the last three tile indices, oldest byte first. All zero means the
 * nametable really is blank there and the fault is upstream of this loop;
 * non-zero means the tiles have content and it is the pattern fetch below that
 * is losing it. */
unsigned long nes_bg_vaddr = 0;
unsigned long nes_bg_xofs  = 0;
unsigned long nes_bg_tail  = 0;
int           nes_bg_line  = -1;

/* The pattern the tile covering x=248..255 was actually built from: the two
 * CHR bytes for this row, and the PPU address they came from. The tile index
 * is known and its cell renders blank, which leaves exactly two possibilities -
 * the pattern really is empty, or it is being fetched from the wrong half of
 * CHR. pat=$0000 says the first; the address says which half. */
unsigned long nes_bg_pat     = 0;
unsigned long nes_bg_pataddr = 0;

/* CHR RAM traffic through $2007. See the write path for what these separate. */
unsigned long nes_chr_writes = 0;
unsigned long nes_chr_max    = 0;
int           nes_bg_nt      = 0;   /* nametable the scanline-32 fetch used */

/* Sprite 0's OAM entry, for the log. Four bytes that say where the game thinks
 * it put the thing it is waiting on. */
void ppu_sprite0(int *y, int *x, int *tile, int *attr)
{
   *y    = ppu.oam[0];
   *tile = ppu.oam[1];
   *attr = ppu.oam[2];
...
/* 1 = background enabled, 2 = sprites enabled. Rendering switched off entirely
 * is its own explanation for a still picture and needs no other. */
int ppu_rendering_state(void)
{
   return (ppu.bg_on ? 1 : 0) | (ppu.obj_on ? 2 : 0);
}

/* WHICH ONE-SCREEN NAMETABLE IS MAPPED RIGHT NOW.
 *
 * page[8] is set to nametab + (n << 10) - 0x2000, so undoing that arithmetic
 * recovers n. On a mapper 7 cart this is bit 4 of the last bank write, and it
 * decides which half of ppu.nametab every $2000-$2FFF access lands in - reads
 * and writes alike. A write logged under one value and a fetch made under the
 * other are two different bytes at the same address, which is exactly what a
 * nametable byte written as $61 and read back as $FF looks like. */
int ppu_nametable_sel(void)
{
   return (int)(((ppu.page[8] + 0x2000) - ppu.nametab) >> 10);
}
   ...
   case PPU_VDATA:
      ...
      { 
         ...
         else
         {
...
#if NES_PPU_DIAGNOSTICS
            /* Every accepted write below $2000 is a byte of pattern data
             * arriving in CHR RAM. The high-water mark says how far up the
             * game's upload actually got: $1F70 holds a tile that draws, $1FF0
             * holds one that comes back blank, and both are in the same 1 KB
             * page - so either the upload stops short of the top, or these
             * writes are not the ones filling it. */
            if (addr < 0x2000)
            {
               nes_chr_writes++;
               if (addr > nes_chr_max)
            ...
            /* THE TWO CELLS THE HANG COMES DOWN TO, traced rather than counted.
             *
             * $1FF0-$1FFF is the pattern of tile $FF, which draws blank; $209F
             * is the nametable byte that put tile $FF over sprite 0 at column
             * 31 of row 4 - caught through any of its mirrors, since a
             * one-screen game may write it through $2400, $2800 or $2C00.
             *
             * Counting could not separate the two ways this ends: the game
             * writing real pattern data that then goes missing, and the game
             * never writing anything but zeros there because tile $FF was never
             * meant to be solid - in which case the nametable byte is the thing
             * that is wrong. Capped so a game that writes here every frame
             * cannot bury the rest of the log. */
            {
               static int chr_shown = 0, nt_shown = 0;

               if (addr >= 0x1FF0 && addr <= 0x1FFF && chr_shown < 48)
               {
                  chr_shown++;
                  log_printf("[CHR] $%04X = $%02X\n", (unsigned)addr, value);
               }
               else if (addr >= 0x2000 && addr < 0x3000
                        && 0x009F == (addr & 0x03FF) && nt_shown < 64)
               {
                  nt_shown++;
                  log_printf("[NT ] $%04X = $%02X  nt=%d\n", (unsigned)addr,
                             value, ppu_nametable_sel());
               }
            }
#endif /* NES_PPU_DIAGNOSTICS */
         }
                        ...
                        uint8 pat2, const uint8 *col_tbl, bool check_strike)
{
   ...
   if (color)
   {
      ...
      if (check_strike)
      {
         /* Reached only inside `if (color)`, so sprite 0 has at least one
          * opaque pixel on this scanline. Counted separately from the strike
          * itself because the two failures need different fixes: a sprite that
          * is never opaque means its pattern never arrived (CHR RAM), while an
          * opaque sprite that never strikes means the background under it is
          * transparent - or is being drawn a scanline out of step. */
#if NES_PPU_DIAGNOSTICS
         nes_spr0_opaque++;

         /* THE TWO SIDES OF THE TEST BELOW, RECORDED SEPARATELY. The strike
          * needs an opaque sprite pixel over a solid background pixel in the
          * same column; opaque= only says the row had one somewhere, which for
          * a sprite hanging off the right edge at x=254 is the wrong question -
          * six of its eight columns are past 255 and can never count.
          *
          * OR'd across every row sprite 0 is drawn on, so one printed pair
          * covers the whole sprite. Bit n is column n of the sprite, left to
          * right on screen. An AND of zero between them is the whole failure,
          * and which side is empty says which half to go and look at. */
         {
            int i;
            for (i = 0; i < 8; i++)
            {
               if (colors[i])            nes_spr0_colmask |= 1u << i;
               if (BG_SOLID(surface[i])) nes_spr0_bgmask  |= 1u << i;
            }
         }
#endif /* NES_PPU_DIAGNOSTICS */
...
#if NES_PPU_DIAGNOSTICS
   const bool bg_capture = (32 == nes_bg_line);
   int bg_seen = 0;

   if (bg_capture)
   {
      nes_bg_vaddr = ppu.vaddr;
      nes_bg_xofs  = ppu.tile_xofs;
      nes_bg_tail  = 0;
      nes_bg_nt    = ppu_nametable_sel();   /* which half the fetch reads */
   }
#endif
...
#if NES_PPU_DIAGNOSTICS
      if (bg_capture && ++bg_seen > 30)     /* the last three of the 33 */
         nes_bg_tail = (nes_bg_tail << 8) | tile_index;
#endif
      data_ptr = &PPU_MEM(bg_offset + (tile_index << 4));

#if NES_PPU_DIAGNOSTICS
      /* The 32nd of 33 is the tile over x=248..255 at xofs=0 - the one holding
       * the pixels sprite 0 is waiting to overlap. */
      if (bg_capture && 32 == bg_seen)
      {
         nes_bg_pat     = ((unsigned long)data_ptr[0] << 8) | data_ptr[8];
         nes_bg_pataddr = (unsigned long)(bg_offset + (tile_index << 4));
      }
#endif
...
#if NES_PPU_DIAGNOSTICS
      if (check_strike)
         nes_spr0_scanned++;   /* sprite 0 is on this scanline and eligible */
#endif
      strike_pixel = draw_oamtile(bmp_ptr, attrib, data_ptr[0], data_ptr[8], ppu.palette + 16 + col_high, check_strike);
      /* SCREEN X, not the offset within the tile. draw_oamtile() returns which
       * of the sprite's eight pixels overlapped, 0 to 7; ppu_setstrike() wants
       * the position along the scanline, because it divides by 3 to turn PPU
       * dots into CPU cycles and work out when $2002 should start reporting the
       * hit. Handing it 0..7 put every strike within two cycles of the start of
       * the scanline - up to 84 cycles early, most of a scanline, so a raster
       * split lands in the wrong place. ppu_fakeoam() below has always passed
       * sprite_x + n; this is the same value. */
      if (strike_pixel >= 0)
         ppu_setstrike(sprite_x + strike_pixel);
...
#if NES_PPU_DIAGNOSTICS
   nes_bg_line = scanline;      /* which line ppu_renderbg() is about to build */
#endif
